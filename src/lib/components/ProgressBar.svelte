...
<div on:click={seekVideo} class="tracker">
	{#each data.points as point}
		...
		<div
			class="point"
			style="left: {(point.startTime / duration) * 100}%; width: {(pointDuration / duration) *
				100}%;"
		></div>
	...
	{#if queuedPoint}
		<div
			class="point-queued"
			style="left: {(queuedPoint.startTime / duration) * 100}%; width: {((currentTime -
				queuedPoint.startTime) /
				duration) *
				100}%;"
		></div>
	{/if}
