	...
	function scrollToCurrentEvent(time) {
		...
		for (let i = 1; i < events.length; i++) {
			if (events[i].time >= time) {
				...
				eventElements[i - 1]?.scrollIntoView({ behavior: 'smooth', block: 'start' });
...
<div style="height: 100%; width: 100%">
	<div style="width: 100%; overflow-y:auto; height: 10em">
				...
				>
					<span>
						...
						{:else if event.type == 'Completion'}
							{shortName(event.primaryPlayer.name)} - {shortName(event.secondaryPlayer.name)}
						{:else if event.type == 'Turnover'}
							{shortName(event.primaryPlayer.name)}
